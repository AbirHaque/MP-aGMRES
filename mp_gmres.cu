...
__global__ void put_vec64_div_scalar64_to_vec32(
    PREC_TYPE* out,
    double* x,
    double scalar,
    int N
...
{  
    int tid = blockDim.x * blockIdx.x + threadIdx.x ;
    if(tid==0){
        PREC_TYPE tmp1=a_i*hi[j]+b_i*hip1[j];
...
{  
    ...
    if(tid==0){
        ...
        PREC_TYPE tmp1,tmp2;
        PREC_TYPE tmp;
        if(fabs(hjp1[j])<0.000001f){
            c=1.0f;
            s=0.0f;
        }
        else if(fabs(hjp1[j])>fabs(hj[j])){
            tmp=hj[j]/hjp1[j];
            s=1.0f/sqrt(1.0f+tmp*tmp);
            c=tmp*s;
        }
        else{
            tmp=hjp1[j]/hj[j];
            c=1.0f/sqrt(1.0f+tmp*tmp);
            s=tmp*c;
        }
        a_[j]=c;
        b_[j]=s;
...
)
{
    ...
    PREC_TYPE* tmp_float_vec=(PREC_TYPE*) calloc(N,sizeof(PREC_TYPE));
    ...
    PREC_TYPE* gpu_A_valuePtr_float;
    ...
    PREC_TYPE* gpu_y;
    PREC_TYPE* gpu_z;
    PREC_TYPE neg_one_float = -1.0f;
    PREC_TYPE one_float = 1.0f;
    PREC_TYPE zero_float = 0.0f;
    double one_double = 1.0;
    double zero_double = 0.0;
    double neg_one_double = -1.0;
    PREC_TYPE* gpu_neg_one_float;
    PREC_TYPE* gpu_one_float;
    PREC_TYPE* gpu_zero_float;
    ...
    PREC_TYPE* y=(PREC_TYPE*) calloc(N,sizeof(PREC_TYPE));


    PREC_TYPE* Q_T[k+1];
    PREC_TYPE* H[k+1];

    PREC_TYPE* Q_T_local[k+1]; //Transpose Q since we only access colums of Q for most of GMRES
    for(i=0;i<k+1;i++){
        Q_T_local[i]=(PREC_TYPE*)calloc(N,sizeof(PREC_TYPE));
    } 
    PREC_TYPE H_local[k+1][k]={0}; 
    PREC_TYPE s_local[k+1]={0};
    ...
    cublasCreate(&handle);
    CHECK_CUDA(cudaMalloc((void**)&gpu_neg_one_float , sizeof(PREC_TYPE)));
    CHECK_CUDA(cudaMalloc((void**)&gpu_one_float     , sizeof(PREC_TYPE)));
    CHECK_CUDA(cudaMalloc((void**)&gpu_zero_float    , sizeof(PREC_TYPE)));
    ...
    CHECK_CUDA(cudaMemset(gpu_neg_one_float , -1.0f, sizeof(PREC_TYPE)));
    CHECK_CUDA(cudaMemset(gpu_one_float     , 1.0f, sizeof(PREC_TYPE)));
    CHECK_CUDA(cudaMemset(gpu_zero_float    , 0.0f, sizeof(PREC_TYPE)));
    ...
    CHECK_CUDA(cudaMalloc((void**)&gpu_A_valuePtr_float, A_num_valuePtr*sizeof(PREC_TYPE)));
    ...
    CHECK_CUDA(cudaMalloc((void**)&gpu_val, sizeof(PREC_TYPE)));
    ...
    CHECK_CUDA(cudaMalloc((void**)&gpu_y, N*sizeof(PREC_TYPE)));
    CHECK_CUDA(cudaMemset(gpu_y, 0, N*sizeof(PREC_TYPE)));
    CHECK_CUDA(cudaMalloc((void**)&gpu_z, N*sizeof(PREC_TYPE)));

    CHECK_CUDA(cudaMalloc((void**)&s,(k+1)*sizeof(PREC_TYPE)));
    CHECK_CUDA(cudaMalloc((void**)&a_,k*sizeof(PREC_TYPE)));
    CHECK_CUDA(cudaMalloc((void**)&b_,k*sizeof(PREC_TYPE)));


    for(i=0;i<k+1;i++){
        CHECK_CUDA(cudaMalloc((void**) &Q_T[i],N*sizeof(PREC_TYPE)));
        CHECK_CUDA(cudaMalloc((void**) &H[i],k*sizeof(PREC_TYPE)));        
    } 
    ...
    CHECK_CUDA(cudaMemcpy(gpu_A_valuePtr_float, A_valuePtr_float, A_num_valuePtr*sizeof(PREC_TYPE), cudaMemcpyHostToDevice));
    ...
    CHECK_CUSPARSE(cusparseCreateCsr(&A_float, N, N, A_num_valuePtr,
                                        gpu_A_outerIndexPtr, gpu_A_innerIndexPtr, gpu_A_valuePtr_float,
                                        CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
    ...
    for(restart_ind=0;restart_ind<max_restarts;restart_ind++){
        ...
        CHECK_CUDA(cudaMemset(s, 0, (k+1)*sizeof(PREC_TYPE)));
        ...
        if(prev_err>=tol*b_norm_double){
            ...
            */
            ...
            put_vec64_div_scalar64_to_vec32<<<N,1>>>( Q_T[0],  gpu_ri_double, r0_norm, N);

            CHECK_CUDA(cudaDeviceSynchronize());
            beta = r0_norm;
            CHECK_CUDA(cudaMemcpy(s,&beta,sizeof(PREC_TYPE),cudaMemcpyHostToDevice));
            ...
            for(j=0;j<k;j++){
                ...
                for(i=0;i<j+1;i++){
                    CHECK_CUBLAS(cublasSdot(handle, N, Q_T[i], 1,Q_T[j+1], 1,H[i]+j));
                    vec_sub_scalar_vec<<<grid_size,block_size>>>(Q_T[j+1],Q_T[i],H[i]+j,gpu_N);

                }
                CHECK_CUBLAS(cublasSnrm2(handle, N,Q_T[j+1], 1, H[j+1]+j));
                ...
                if(sjp1<tol*b_norm_double){
                    for(i=0;i<k+1;i++){
                        CHECK_CUDA(cudaMemcpy(Q_T_local[i],Q_T[i],N*sizeof(PREC_TYPE),cudaMemcpyDeviceToHost));
                        CHECK_CUDA(cudaMemcpy(H_local[i],H[i],k*sizeof(PREC_TYPE),cudaMemcpyDeviceToHost));
                    } 
                    CHECK_CUDA(cudaMemcpy(s_local,s,(k+1)*sizeof(PREC_TYPE),cudaMemcpyDeviceToHost));
...
int main(int argc, char *argv[]){
    srand(0);
    int i,j,k,l,m,n,N;
	string filename = argv[1];//"./pores_1.mat";
    ...
    PREC_TYPE* A_valuePtr_float=(PREC_TYPE*) malloc(A_num_valuePtr*sizeof(PREC_TYPE));
    ...
    for(i=0;i<N;i++){b[i]=/*distro(gen)*/(((double)rand())/RAND_MAX);}//VectorXd::Ones(N);
    double* x0 = (double*) calloc(N,sizeof(double));//VectorXd::Zero(N);
    double* x_hat = (double*) calloc(N,sizeof(double));//VectorXd::Zero(N);
    k=stoi(argv[2]); //N-10;
    double tol= stod(argv[3]); //1e-6;
    int max_restarts= stoi(argv[4]);//10;
    int iters;
    int restarts;
